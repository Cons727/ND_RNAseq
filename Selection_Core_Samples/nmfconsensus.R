#https://github.com/syspremed/PanNETassigner

nmfconsensus <-function(input.ds, k.init, k.final, num.clusterings, maxniter, error.function, rseed=123456789, stopconvergence = 40, stopfrequency = 10, non.interactive.run = F, doc.string = "",data=FALSE, ...) {
  ...
  ######################################################################################
  ######### Include all functions required by nmfconcensus function here
  ######################################################################################
  ...
  col.names <- colnames(D)   # edited  to colnames instead of names(D)
